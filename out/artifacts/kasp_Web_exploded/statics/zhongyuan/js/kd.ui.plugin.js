$.extend({
	...
	kd: {
		...
		kdWin: function (width, height, title, contentInfo, backcallok, type, contentType, hidebg, undrag, backcallcancel, okTxt, cancelTxt, onload) {
			...
			winHTML.push("<div id='kd-ui-dialog-bottom'>", (type > 0 ? "<input id='kduiokbtn' type='button' class='kdmall-btn cancel' onclick='$(\"#kd-ui-dialog-close\").click();if(typeof $.kd.kdWin.backcallok == \"function\") $.kd.kdWin.backcallok();' value='" + (okTxt ? okTxt : "\u786E\u5B9A") +　"'>" : ""), (type == 2 ? "<input type='button' class='kdmall-btn' style='margin-left: 8px;' onclick='$(\"#kd-ui-dialog-close\").click();if(typeof $.kd.kdWin.backcallcancel == \"function\") $.kd.kdWin.backcallcancel();' value='" + (cancelTxt ? cancelTxt : "\u53D6\u6D88") +　"'>" : ""), "</div>");
			winHTML.push("</div>");
		...
		num2china:function(money){
            if(!/^\d*(\.\d*)?$/.test(money)) return "非数字";
            //var moneyArray = money.split(".");
            var _chinaNum  = new Array("零", "壹", "贰", "叁", "肆", "伍", "陆", "柒", "捌", "玖"); 
            var _chinaUnit = new Array("", "拾", "佰", "仟", "万", "億", "点", "");
            var a = ("" + money).replace(/(^0*)/g, "").split("."), k = 0, re = "";
            ...
            if(a[0].length == 0){
                ...
                for (var i = a[0].length - 1; i >= 0; i--) { 
                    ...
                    if (a[0].charAt(i) != 0) 
                        re = _chinaNum[a[0].charAt(i)] + _chinaUnit[k % 4] + re; 
