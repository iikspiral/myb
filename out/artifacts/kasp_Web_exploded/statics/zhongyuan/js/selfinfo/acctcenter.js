...
$(function() {
    ...
    //打开验证码模态框，获取手机验证码
    function checkcode(wait, $btn,mobile,type) {
    ...
    $("#lgnphonecode").click(function() {
        ...
        checkcode(300, $("#lgnphonecode"),"","lgpwd");
    });
    //获取验证码，修改交易密码
    $("#trdphonecode").click(function() {
        ...
        checkcode(300, $("#trdphonecode"),"","tapwd");
    });
    //获取验证码，修改手机
    $("#chgphonecode").click(function() {
        ...
        var url = 'mall/zhongyuan_checkAction!phoneExsits.do';
        $.post(url, {
            phonenum: phonenum
        }, function(data) {
            var data = eval("(" + data + ")");
            if (data.result == '1') {
                checkcode(300, $("#chgphonecode"),phonenum,"phcode");
            } else {
...
function toNextStep4login(formid, showid) {
    ...
    var url = 'mall/zhongyuan_validAcion!checkPhoneCode.do';
...
function updLoginPwd(formid, showid) {
    ...
    var desPwd = $.des.getDes(_pwd, 'kingdom');
    $("#password").val(desPwd);
    $("#password2").val(desPwd);
    var url = "mall/zhongyuan_set_custPassword!resetLoginPwd.do";
    $.post(url, $('#' + formid).serializeArray(), function(data, textStatus, xhr) {
        var data = eval("(" + data + ")");
        if (data.result == '1') {
          //  nextstep(formid, showid);
		  
            zhejiang_alert('提示', "登录密码重置成功，您需要重新登录！", function(){
...
function updTradePwd(formid, showid) {
   ...
   var url = "mall/zhongyuan_set_custPassword!resetTradePwd.do";
    $.post(url, {
        npassword: desPwd,
		phonecode:phonecode
...
function updTradePwd(formid, showid) {
    ...
    $.post(url, {
        ...
        var data = eval("(" + data + ")");
        if (data.result == '1') {		   
            zhejiang_alert('提示', "交易密码重置成功", function(){
				 timeOutRelocate(1, 'security.html', 'successTPD');
			//	window.location.href='security.html';
			});
...
//登出
function logout() {
    $.post("mall/zhongyuan_login!newlogout.do", {}, function(data) {
...
function updSafetyQuestion(formid, showid) {
    ...
    var url = "mall/zhongyuan_saftyQuestion!setQuestion.do";
    $.post(url, $("#" + formid).serializeArray(), function(data) {
        var data = eval("(" + data + ")");
        if (data.result == '1') {
			 zhejiang_alert('提示', "密保设置成功", function(){
				 timeOutRelocate(1, 'security.html', 'relocation');
			});
        } else {
            zhejiang_alert('提示', data.message, refreshpage);
        }
...
function validQuestion(formid, showid) {
    ...
    var url = "mall/zhongyuan_validQuestion!validate4ChangeMobile.do";
    $.post(url, $formid.serializeArray(), function(data) {
        var data = eval("(" + data + ")");
        if (data[0].success) {
			setphone(formid, showid);
        } else {
            $('#loginpwd', $formid).val('');
...
function setphone(formid, showid) {
    ...
    if (!flag) {
        return
         ...
         var url = "mall/zhongyuan_setCustPhone!setphone.do";
    $.post(url, $("#" + formid).serializeArray(), function(data) {
        var data = eval("(" + data + ")");
        if (data.result == '1') {
			
			 zhejiang_alert('提示', "手机号换绑成功！", function(){
				 timeOutRelocate(1, 'security.html', 'changephone');
			});
        } else {
            zhejiang_alert('提示', data.message, refreshpage);
        }
...
//发送手机验证码
function sendcode(wait, $btn,mobile,type) {
    var validcode = $.trim($("#validcode").val());
    var mobiletel = (mobile == "")?$.trim($("#mobiletel").val()):mobile;
    $.post('mall/zhongyuan_sendPhoneCode!sendPhoneCode.do', {
        validcode: validcode,
        mobiletel: mobiletel,
        codetype:type
    }, function(data) {
...
function sendEmail() {
    ...
    var url = 'mall/zhongyuan_email!sendEmail.do';
...
function addCustOtherInfo() {
    var url = "mall/zhongyuan_addCust!addCust.do";
    $.post(url, $('#otherinfo').serializeArray(), function(data) {
        //console.log(data);
        var data = eval('(' + data + ')');
        zhejiang_alert('提示', data.msg, refreshpage);
    });
}
//刷新验证码图片
...
function _dialog(wait, $btn,mobile,type) {
...
Project.Basic = {
    ...
    changeCity: function() {
        ...
        if (selectedProvince != undefined || selectedProvince != -1) {
            $.get("mall/zhongyuan_provincedict!getCitiesByProvince.do", {
                ajax: "yes",
...
function submitBindAct(formid, showid, obj) {
    ...
    $.post(url, $("#acctform").serializeArray(), function(data) {
        ...
        if (data.result == '1') {
            ...
            nextstep(formid, showid);
            $(obj).hide();
            timeOutRelocate(3, 'security.html', '');
        } else {
            zhejiang_alert('提示', data.message);
...
function submitBindCap(formid, showid, obj) {
    ...
    var despwd = $.des.getDes(opassword, 'kingdom');
    //if ($("#" + formid).kdValidform().check()) return;
...
$(".btnSubmitFXCP").bind("click",function(){
    ...
    var url = 'mall/zhongyuan_riskmate!doEstimate.do?ajax=yes';
