...
/* 往页面添加自定义名称按钮的弹出框 */
var addPopUpdefined = function(id,title,msg,choose,btn1,btn2){
	if(choose){//有函数，无按钮
		$("body")
		.append('<div class="modal fade" id="'+id+'" tabindex="-1" role="dialog"  aria-labelledby="myModalLabel" aria-hidden="true">'
			+'<div class="modal-dialog" style="width: 400px;">'
				+'<div class="modal-content">'
					...
					+'</div>'
					+'<div class="modal-footer">'
						+'<a type="button" class="btn-sm btn-orange btn" id="'+id+'_sure">'+btn1+'</a>'
						
						+"&nbsp;&nbsp;"
						+'<a type="button" class="btn-sm btn-grey btn-adjust" id="'+id+'_cancel">'+btn2+'</a>'
...
var kp_confirm = function(title,msg,func,funcs,btn1,btn2){
	var id = 'confirmWindow';
	if( typeof(func) == 'function'){
		// 往页面添加弹出框
		addPopUpdefined(id,title,msg,true,btn1,btn2);
		$('#'+id).modal({backdrop: 'static', keyboard: false});
		// 显示弹出框
		$('#'+id).modal('show');
		// 绑定确定按钮事件
