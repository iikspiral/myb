	...
	kd: {
		...
		kdPager: {
			...
			generPageHtml: function() {
				...
				if (this.hasPrv) {
					str_prv.push("<a class='prev' href='", this.getLink(this.prv), "' title='上一页'>上一页</a>");
				} else {
					str_prv.push("<span class='disabled prev'>上一页</span>");
				}
	
				if (this.hasNext) {
					str_next.push("<a class='next' href='", this.getLink(this.next), "' title='下一页'>下一页</a>");
				} else {
					str_next.push("<span class='disabled next'>下一页</span>");
				}
	
				var str = [], dot = '<span>...</span>', total_info = [];
				if (this.isShowTotalPage || this.isShowTotalRecords) {
					total_info.push("<span class='normalsize'>共");
					if (this.isShowTotalPage) {
						total_info.push(this.totalPage, "页");
						this.isShowTotalRecords && total_info.push("&nbsp;/&nbsp;");
					}
					this.isShowTotalRecords && total_info.push(this.totalRecords, "条记录");
				...
				if (this.isGoPage) {
					gopage_info.push(
						...
						"<span id='go_page_wrap' class='goto'>",
							"<input id='btn_go' type='button' class='gotobutton' onclick='$.kd.kdPager.gopage();' value='确定' />",
							"<input id='btn_go_input' type='text' class='gotoinput' onfocus='$.kd.kdPager.inpage()' onkeypress='$.kd.kdPager.checkKey(event)' onblur='$.kd.kdPager.outpage()' value='", this.next, "' />",
						"</span>",
				...
				if (this.totalPage <= 8) {
					for (var i = 1; i <= this.totalPage; i++) {
						if (this.currentPage == i) {
							str.push("<span class='curr'>", i,"</span>");
						} else {
							str.push("<a href='", this.getLink(i), "' title='第", i, "页'>", i, "</a>");
						}
					}
				} else {
					if (this.currentPage <= 5) {
						...
						str.push(dot);
						str.push("<a href='", this.getLink(this.totalPage), "' title='第", this.totalPage, "页'>", this.totalPage, "</a>");
					} else {
						str.push("<a href='", this.getLink(1), "' title='第1页'>1</a>");
						str.push(dot);
				...
				$("#" + this.pager).html("<div class='page_nav'>" + str_prv.join("") + str.join("") + str_next.join("") + total_info.join("") + gopage_info.join("") + "</div>");
			}
		},
		...
		kdWin: function (width, height, title, contentInfo, backcallok, type, contentType, hidebg, undrag, backcallcancel, okTxt, cancelTxt, onload, beforeClose) {
			$.kd.kdWin.backcallok = backcallok;
			$.kd.kdWin.backcallcancel = backcallcancel;
			$.kd.kdWin.beforeClose = beforeClose;
			...
			var winHTML = []			
			winHTML.push("<div id='kd-ui-dialog-bg' style='height: ", $(document).height(), "px;filter: alpha(opacity=0);opacity: 0;z-index: 100000';", (hidebg ? "display: none" : ""), "></div>");
			winHTML.push("<div id='kd-ui-dialog-box'>");
			winHTML.push("<div id='kd-ui-dialog-title'><h2>", title, "</h2><span id='kd-ui-dialog-close'>&times;</span></div>");
			winHTML.push("<div id='kd-ui-dialog-content-border'><div id='kd-ui-dialog-content'></div></div>");
			winHTML.push("<div id='kd-ui-dialog-bottom'>", (type > 0 ? "<input id='kduiokbtn' type='button' class='kdmall-btn' onclick='if(typeof $.kd.kdWin.beforeClose != \"function\" || $.kd.kdWin.beforeClose()) {if(typeof $.kd.kdWin.backcallok == \"function\") $.kd.kdWin.backcallok();$(\"#kd-ui-dialog-close\").click();}' value='" + (okTxt ? okTxt : "\u786E\u5B9A") + "'>" : ""), (type == 2 ? "<input type='button' class='kdmall-btn' style='margin-left: 8px;' onclick='if(typeof $.kd.kdWin.backcallcancel == \"function\") $.kd.kdWin.backcallcancel();$(\"#kd-ui-dialog-close\").click();' value='" + (cancelTxt ? cancelTxt : "\u53D6\u6D88") + "'>" : ""), "</div>");
			winHTML.push("</div>");
			...
			$("#kd-ui-dialog-bg").css({opacity: "0.5"});
			
			if (height >= 527) {
				$("#kd-ui-dialog-title").css("width", parseInt(width) + 22);
				$("#kd-ui-dialog-content").css("width", parseInt(width) + 10);
			} else {
				$("#kd-ui-dialog-title").css("width", parseInt(width) + 10);
		...
		kdAlert: function(content, backcall, title) {
			$.kd.kdWin(400, 150, title ? title : "\u8B66\u544A", content, backcall, 1);
		},
