...
$(function() {
    ...
    $("#lgnphonecode").click(function() {
        ...
        checkcode(60, $("#lgnphonecode"),"","lgpwd");
    });
    //获取验证码，修改交易密码
    $("#trdphonecode").click(function() {
        ...
        checkcode(60, $("#trdphonecode"),"","tapwd");
    });
    //获取验证码，修改手机
    $("#chgphonecode").click(function() {
        ...
        var url = 'mall/hunan_checkAction!phoneExsits.do';
        $.post(url, {
            phonenum: phonenum
        }, function(data) {
            var data = eval("(" + data + ")");
            if (data.result == '1') {
                checkcode(60, $("#chgphonecode"),phonenum,"phcode");
            } else {
...
function toNextStep4login(formid, showid) {
    ...
    var url = 'mall/hunan_validAcion!checkPhoneCode.do';
...
function updLoginPwd(formid, showid) {
    ...
    var url = "mall/hunan_set_custPassword!resetLoginPwd.do";
...
function updTradePwd(formid, showid) {
    ...
    var url = "mall/hunan_set_custPassword!resetTradePwd.do";
    $.post(url, {
        npassword: desPwd,
		phonecode:phonecode
...
//登出
function logout() {
    $.post("mall/hunan_login!newlogout.do", {}, function(data) {
...
function updSafetyQuestion(formid, showid) {
    ...
    var url = "mall/hunan_saftyQuestion!setQuestion.do";
    $.post(url, $("#" + formid).serializeArray(), function(data) {
        var data = eval("(" + data + ")");
        if (data.result == '1') {
            nextstep(formid, showid);
...
function validQuestion(formid, showid) {
    ...
    var url = "mall/hunan_validQuestion!validate4ChangeMobile.do";
    $.post(url, $formid.serializeArray(), function(data) {
        var data = eval("(" + data + ")");
        if (data[0].success) {
            nextstep(formid, showid);
...
function setphone(formid, showid) {
    ...
    var url = "mall/hunan_setCustPhone!setphone.do";
    $.post(url, $("#" + formid).serializeArray(), function(data) {
        var data = eval("(" + data + ")");
        if (data.result == '1') {
            nextstep(formid, showid);
...
function sendcode(wait, $btn,mobile,type) {
    ...
    $.post('mall/hunan_sendPhoneCode!sendPhoneCode.do', {
        validcode: validcode,
        mobiletel: mobiletel,
        codetype:type
    }, function(data) {
...
function sendEmail() {
    ...
    var url = 'mall/hunan_email!sendEmail.do';
...
function addCustOtherInfo() {
    var url = "mall/hunan_addCust!addCust.do";
...
Project.Basic = {
    ...
    changeCity: function() {
        ...
        if (selectedProvince != undefined || selectedProvince != -1) {
            $.get("mall/hunan_provincedict!getCitiesByProvince.do", {
                ajax: "yes",
...
$(".btnSubmitFXCP").bind("click",function(){
    ...
    var url = 'mall/hunan_riskmate!doEstimate.do?ajax=yes';
