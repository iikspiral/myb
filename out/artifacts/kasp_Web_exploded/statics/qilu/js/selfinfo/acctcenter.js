...
$(function() {
    ...
    $("#chgphonecode").click(function() {
        ...
        var url = 'mall/qilu_checkAction!phoneExsits.do';
        $.post(url, {
            phonenum: phonenum
        }, function(data) {
            var data = eval("(" + data + ")");
...
function toNextStep4login(formid, showid) {
    ...
    var url = 'mall/qilu_validAcion!checkPhoneCode.do';
...
function updLoginPwd(formid, showid) {
    ...
    var url = "mall/qilu_set_custPassword!resetLoginPwd.do";
...
function updTradePwd(formid, showid) {
    ...
    var url = "mall/qilu_set_custPassword!resetTradePwd.do";
    $.post(url, {
        npassword: desPwd
...
//登出
function logout() {
    $.post("mall/qilu_login!newlogout.do", {}, function(data) {
...
function updSafetyQuestion(formid, showid) {
    ...
    var url = "mall/qilu_saftyQuestion!setQuestion.do";
    $.post(url, $("#" + formid).serializeArray(), function(data) {
        var data = eval("(" + data + ")");
        if (data.result == '1') {
            nextstep(formid, showid);
...
function validQuestion(formid, showid) {
    ...
    var url = "mall/qilu_validQuestion!validate4ChangeMobile.do";
    $.post(url, $formid.serializeArray(), function(data) {
        var data = eval("(" + data + ")");
        if (data[0].success) {
            nextstep(formid, showid);
...
function setphone(formid, showid) {
    ...
    var url = "mall/qilu_setCustPhone!setphone.do";
    $.post(url, $("#" + formid).serializeArray(), function(data) {
        var data = eval("(" + data + ")");
        if (data.result == '1') {
            nextstep(formid, showid);
...
function sendcode(wait, $btn,mobile,type) {
    ...
    $.post('mall/qilu_sendPhoneCode!sendPhoneCode.do', {
        validcode: validcode,
        mobiletel: mobiletel,
        codetype:type
    }, function(data) {
...
function sendEmail() {
    ...
    var url = 'mall/qilu_email!sendEmail.do';
...
function addCustOtherInfo() {
    var url = "mall/qilu_addCust!addCust.do";
...
Project.Basic = {
    ...
    changeCity: function() {
        ...
        if (selectedProvince != undefined || selectedProvince != -1) {
            $.get("mall/qilu_provincedict!getCitiesByProvince.do", {
                ajax: "yes",
...
$(".btnSubmitFXCP").bind("click",function(){
    ...
    var url = 'mall/qilu_riskmate!doEstimate.do?ajax=yes';
