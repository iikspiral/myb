...
public class Page<T> {
	...
	public String toString() {
		...
		sb.append("<input type=\"text\" value=\""+(count%pageSize==0?(count/pageSize):(count/pageSize+1))+"\" onkeypress=\"var e=window.event||this;var c=e.keyCode||e.which;if(c==13)");
...
//		sb.insert(0,"<div class=\"page\">\n").append("</div>\n");
	...
	public String getJinRunPage() {

		StringBuilder sb = new StringBuilder();
		
		if (pageNo == first) {// 如果是首页
			sb.append("<a href=\"javascript:\"><div class=\"before\">上一页</div></a>");
		} else {
			sb.append("<a href=\"javascript:\" onclick=\""+funcName+"("+prev+","+pageSize+",'"+funcParam+"');\"><div class=\"before\">上一页</div></a>");
		}
		...
		if (begin > first) {
			int i = 0;
			for (i = first; i < first + slider && i < begin; i++) {
				sb.append("<a href=\"javascript:\" onclick=\""+funcName+"("+i+","+pageSize+",'"+funcParam+"');\"><div class=\"match\">" + (i + 1 - first) + "</div></a>");
			}
			if (i < begin) {
				sb.append("<a href=\"javascript:\"><div class=\"match\">...</div></a>");
			}
		}

		for (int i = begin; i <= end; i++) {
			if (i == pageNo) {
				sb.append("<a href=\"javascript:\"><div class=\"match active\">" + (i + 1 - first) + "</div></a>");
			} else {
				sb.append("<a href=\"javascript:\" onclick=\""+funcName+"("+i+","+pageSize+",'"+funcParam+"');\"><div class=\"match\">" + (i + 1 - first) + "</div></a>");
			}
		}
		...
		if (pageNo == last) {
			sb.append("<a href=\"javascript:\"><div class=\"after\">下一页</div></a>");
		} else {
			sb.append("<a href=\"javascript:\" onclick=\""+funcName+"("+next+","+pageSize+",'"+funcParam+"');\"><div class=\"after\">下一页</div></a>");
		}
