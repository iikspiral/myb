...
//		File file = new File(Global.getConfig("AttachmentsDir")+File.separator+fileName);		
	 ...
	 * @throws IOException
	 */
	@RequestMapping(value="${adminPath}/uploadAPK",method = RequestMethod.POST)
	@ResponseBody
	public String uploadAPK(String sysuserid,HttpServletResponse response,HttpServletRequest request, Model model) throws IOException{ 
		String responseStr="";
 		MultipartHttpServletRequest multipartRequest = (MultipartHttpServletRequest) request; 
		Map<String, MultipartFile> fileMap = multipartRequest.getFileMap();
		/**获取文件夹*/
		String parameter = request.getParameter("folderName");
		...
		for(Map.Entry<String, MultipartFile> entity :fileMap.entrySet()){
			...
			path += "upload"+File.separator+folderName+File.separator;
			filePath += "/upload/"+folderName+"/";
			File folder = new File( path );
			if ( !folder.exists() )
				folder.mkdirs();
			File uploadFile = new File(path+"miyaobao.apk");
			try {
				FileCopyUtils.copy(mf.getBytes(), uploadFile);
				responseStr = basePath+filePath+"miyaobao.apk;"+id;
			} catch (Exception e) {
				responseStr="上传失败";
				e.printStackTrace();
			}
